...
function RouteComponent() {
  ...
  interface workspaceSchema {
    workspace:{id: string, name: string, img: string, memberLimit: number, type: "professional" | "social"},
  ...
  const {data, isLoading, error} = useQuery({
    queryKey:["workspace"],
    queryFn: async() => {
      const response = await fetch(`${API_URL}/api/workspaces/by-ids?type=professional`, {
        credentials: 'include',
        headers:{
    ...
    <>
      ...
      <TabBar>
        ...
        {data.map((index: workspaceSchema) =>(
          <Link 
            to="/console/workspace/job/$workspaceId" 
